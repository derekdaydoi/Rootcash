(function (root, factory) {
  ...
  function calculateBuffer(incomes, expenses, safetyRate = 10) {
    const events = [];
    ...
    const totalExpense = sum(expenses); const timingGap = Math.max(0, -minimum);
    const safetyMargin = totalExpense * clamp(Number(safetyRate || 0), 0, 100) / 100;
    const recommended = roundUp(timingGap + safetyMargin, 500000);
    return { timingGap, safetyMargin, recommended, totalIncome: sum(incomes), totalExpense, flexible: Math.max(0, sum(incomes) - totalExpense), endingNet: running, events };
  }
